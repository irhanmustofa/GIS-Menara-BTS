<?php 
include "header.php";
$kecamatan = array();
$ambil_kecamatan = $koneksi -> query("SELECT * FROM kecamatan LEFT JOIN kabupaten ON kecamatan.id_kabupaten = kabupaten.id_kabupaten");
while ($tiap_kecamatan = $ambil_kecamatan -> fetch_assoc()){
  $kecamatan[]=$tiap_kecamatan;
}
$kabupaten = array();
$ambil_kabupaten = $koneksi -> query("SELECT * FROM kabupaten");
while ($tiap_kabupaten = $ambil_kabupaten -> fetch_assoc()){
  $kabupaten[]=$tiap_kabupaten;
}
?>
<div class="row">
  <div class="card mt-4" >
    <h4 class="card-title mt-3 ms-3">Daftar Kecamatan</h4>
    <p class="card-category ms-3">Data Daftar Kecamatan</p>

    <div class="card-body">
      <div class="d-grid gap-2 d-md-flex justify-content-md-end pb-3">
        <div class="d-grid gap-2 d-md-flex justify-content-md-end pb-3">
          <!-- Button trigger modal -->
          <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#staticBackdrop">
            <i class="bi bi-plus"></i> Tambah Kecamatan
          </button>

          <!-- Modal -->
          <div class="modal fade" id="staticBackdrop" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title mt-3 ms-3">Input Data Kecamatan</h4>
                </div>
                <div class="modal-body">
                  <form method="post">
                    <div class="mb-3">
                      <label class="form-label">Kabupaten/Kota</label>
                      <select class="form-control" name="kabupaten" required="required">
                        <option value="">--Pilih Kabupaten/Kota</option>
                        <?php foreach ($kabupaten as $key => $value): ?>
                          <option value="<?php echo $value["id_kabupaten"]; ?>"><?php echo $value["nama_kabupaten"]; ?></option>
                        <?php endforeach ?>
                      </select>
                    </div>
                    <div class="mb-3">
                      <label class="form-label">Kode Kecamatan</label>
                      <input type="text" class="form-control" name="kode_kecamatan" required="required">
                    </div>
                    <div class="mb-3">
                      <label class="form-label">Nama Kecamatan</label>
                      <input type="text" class="form-control" name="nama_kecamatan" required="required">
                    </div>
                    <div class="d-grid gap-2 d-md-block text-end">
                      <button class="btn btn-primary" name="simpan">Simpan</button>
                      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    </div>
                  </form>
                  <?php 

                  if (isset($_POST["simpan"])) {
                    $kab = $_POST["kabupaten"];
                    $kode = $_POST["kode_kecamatan"];
                    $nama = $_POST["nama_kecamatan"];


                    $koneksi -> query("INSERT INTO kecamatan(id_kabupaten,kode_kecamatan,nama_kecamatan) VALUES ('$kab','$kode','$nama')");
                    echo "<script>alert ('Data ditambahkan')</script>";
                    echo "<script>location = 'kecamatan.php'</script>";
                  }

                  ?>
                </div>
              </div>
            </div>
          </div>
        </div>

      </div>
      <div class="table table-responsive">
        <table id="datatabel" class="table table-bordered table-striped table-sm table-hover">
          <thead>
            <tr>
              <th>No</th>
              <th>Kabupaten/Kota</th>
              <th>Kode Kecamatan</th>
              <th>Nama Kecamatan</th>
              <th>Aksi</th>
            </tr>
          </thead>
          <tbody>
            <?php foreach ($kecamatan as $key => $value): ?>

              <tr>
                <td><?php echo $key+1; ?></td>
                <td><?php echo $value["nama_kabupaten"]; ?></td>
                <td><?php echo $value["kode_kecamatan"]; ?></td>
                <td><?php echo $value["nama_kecamatan"]; ?> </td>
                <td>
                  <a href="kecamatan_ubah.php?id=<?php echo $value["id_kecamatan"]; ?>" class="btn-sm btn btn-primary text-white" data-bs-toggle="tooltip" title="Ubah">
                    <span class="bi bi-pencil-square"></span>
                  </a>
                  <a href="kecamatan_hapus.php?id=<?php echo $value["id_kecamatan"]; ?>" onclick="return confirm('Apakah anda yakin ingin menghapus data?')" class="btn-sm btn btn-danger" data-bs-toggle="tooltip" title="Hapus">
                    <span class="bi bi-trash-fill"></span>
                  </a>
                </td>
              </tr>

            <?php endforeach ?>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</div>
<?php 
include "footer.php";
?>